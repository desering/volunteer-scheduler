...
import type { PastEventsForUserId } from "@/lib/services/get-past-events-for-user-id";

type Props = {
  initialData?: PastEventsForUserId;
};

export const PastEventsList = (props: Props) => {
  const [selectedEvent, setSelectedEvent] = useState<Event>();
  const [isDrawerOpen, setIsDrawerOpen] = useState(false);

  const { data, refetch } = useQuery<PastEventsForUserId>({
    queryKey: ["events", "users", "upcoming", props.initialData?.events],
    queryFn: async () =>
      await fetch(`/api/events/users/past`).then((res) => res.json()),
      ...
      <Flex flexDirection="column" gap="4">
        {eventsList && eventsList.length > 0 ? (
          ...
          <panda.p textAlign="center">
            There are no past shifts in your history. Go to{" "}
